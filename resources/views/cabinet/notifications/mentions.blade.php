...
{{--                @yield('add_error_block')--}}

{{--                @yield('add_notification_menu')--}}

@section('mentions_tab_active', 'active-navbtn')

@section('notifications_route')
    {{ route('show.mentions.notifications') }}
@endsection
    ...
    <table class="table">
        @foreach($notifications->getCollection() as $notification)
            <tr @empty($notification['read_at']) class="new" @endempty>
                ...
                <td>
                    <p>{{ 'Пользователь "' . \App\User::find($notification['data']['sender'])->login . '" оставил отзыв продавцу "' .
                                            \App\User::find($notification['data']['recipient'])->login . '"' }}</p>
                </td>
                <td class="text-right">
                    <em>{{ new Carbon\Carbon($notification['data']['created_at']) }}</em></td>
            </tr>
        @endforeach
    </table>
@endsection

{{--                @yield('add_pagination_block')--}}
